...
const layoutSchemaId =
  "https://inaku-Gyan.github.io/Live-Serial-Plotter/schemas/layout.schema.json";
const repoRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..");
const schemaPath = path.join(repoRoot, "schemas/layout.schema.json");
...
if (process.argv.includes("--check")) {
  ...
  if (current !== serialized) {
    console.error(
      "schemas/layout.schema.json is out of date. Run `pnpm schema:generate` and commit the result.",
    );
    process.exitCode = 1;
  }
} else {
...
function createLayoutSchema() {
  const generator = createGenerator({
    path: path.join(repoRoot, "src/profiles/layoutSchemaTypes.ts"),
  ...
  const generated = generator.createSchema("LayoutConfigFile");
  const generatedSchema = preferOneOf(generated);

  return {
    $schema: "http://json-schema.org/draft-07/schema#",
    $id: layoutSchemaId,
    title: "Live Serial Plotter Layout",
    description: "Schema for Live Serial Plotter monitor layout JSONC files.",
    ...generatedSchema,
  };
}
